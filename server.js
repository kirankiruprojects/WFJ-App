...
const { TERMINATION_SECTIONS, CRF_SECTIONS, CATEGORY_MATRIX, CATEGORY_OPTIONS, TRACKING_FIELDS, TEAM_NAMES, IMPLEMENTATION_FIELDS, TERMINATION_EXTRA_FIELDS } = require('./schema');
...
(function migrateTypeConstraint() {
  ...
  if (row && row.sql && (!row.sql.includes('implementation') || !row.sql.includes('draft'))) {
    db.exec('BEGIN');
    try {
...
app.get('/api/schema', (req, res) => {
  ...
  res.json({ TERMINATION_SECTIONS, CRF_SECTIONS, CATEGORY_MATRIX, CATEGORY_OPTIONS, TRACKING_FIELDS, TEAM_NAMES: dynamicTeams, STAGES, IMPLEMENTATION_FIELDS, TERMINATION_EXTRA_FIELDS });
});

// ---------- API: submissions ----------

app.get('/api/submissions', (req, res) => {
  const rows = db.prepare('SELECT * FROM submissions WHERE status != \'draft\' AND is_deleted = 0 ORDER BY updated_at DESC').all();
  res.json(rows.map(row => {
    const tasks = getTasks(row.id);
    const header = safeParse(row.header_json, {});
    const taskText = tasks.map(t => (t.label || '') + ' ' + (t.notes || '')).join(' ').toLowerCase();
    return {
      id: row.id, type: row.type, client: row.client, broker: row.broker,
      status: row.status, refConversation: header.refConversation || '',
      progress: progressOf(tasks, row.status), createdAt: row.created_at, updatedAt: row.updated_at,
      taskText
    };
  }));
});
...
app.post('/api/submissions', (req, res) => {
  ...
  if (!['termination', 'crf', 'implementation'].includes(type)) return res.status(400).json({ error: 'invalid type' });
  const id = uid();
  const now = nowIso();
      ...
      for (let r = 0; r < grid.length; r++) {
        ...
        row.forEach((cell, cIdx) => {
          ...
          if (type === 'crf') {
            ...
            else if (cellStr === 'partnername' || cellStr === 'brokername') tempIndices['broker'] = cIdx;
            else if (cellStr === 'conversation#') tempIndices['refConversation'] = cIdx;
            else if (cellStr === 'changerequestedby') tempIndices['requestedBy'] = cIdx;
            else if (cellStr === 'changerequestdate') tempIndices['dateOfRequest'] = cIdx;
            else if (cellStr === 'category') tempIndices['category'] = cIdx;
            else if (cellStr === 'reasonforraising') tempIndices['requestText'] = cIdx;
            else if (cellStr === 'changerequest') tempIndices['proposedSolution'] = cIdx;
            else if (cellStr === 'timespentonconfiguration') tempIndices['timeConfig'] = cIdx;
            else if (cellStr === 'timespentonreview/testing') tempIndices['timeTesting'] = cIdx;
            else if (cellStr === 'no.oferrors' || cellStr === 'no.errors') tempIndices['errors'] = cIdx;
            else if (cellStr === 'configurationanalyst') tempIndices['configAnalyst'] = cIdx;
            else if (cellStr === 'review/testinganalyst') tempIndices['testingAnalyst'] = cIdx;
            else if (cellStr === 'implementationmanager/crm') tempIndices['implementationManager'] = cIdx;
            else if (cellStr === 'completeddate') tempIndices['completedOn'] = cIdx;
            else if (cellStr === 'rating') tempIndices['rating'] = cIdx;
            else if (cellStr === 'comments') tempIndices['comments'] = cIdx;
            else if (cellStr === 'billable/nonbillable') tempIndices['billable'] = cIdx;
          } else if (type === 'implementation') {
            if (cellStr === 'clientname') tempIndices['client'] = cIdx;
